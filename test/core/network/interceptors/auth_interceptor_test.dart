...
void main() {
  ...
  group('AuthInterceptor Refresh Logic', () {
    test('successfully refreshes token and retries request on 401', () async {
      ...
      ).thenAnswer((_) async => 'new_valid_token');
      ...
      verify(
        () => mockLocalDataSource.saveAccessToken('new_valid_token'),
      ).called(1);
      ...
      expect(
        mockRetryDio.interceptors.any(
          (interceptor) => interceptor is MetadataUnwrapInterceptor,
        ),
        isTrue,
      );
